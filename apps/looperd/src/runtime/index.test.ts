...
describe("createLooperdRuntime", () => {
  ...
  test("processes claimed scheduler items concurrently up to maxConcurrentRuns", async () => {
    const fixture = await createFixture();
    fixture.config.agent.vendor = "opencode";
    ...
    for (const id of ["1", "2"]) {
      ...
      store.tasks.upsert({
        ...
        description: null,
        status: "in_progress",
        loopId: `loop_worker_${id}`,
        repo: "powerformer/looper",
        prNumber: null,
        metadataJson: JSON.stringify({ specPath: `spec-${id}.md` }),
        createdAt: now,
        updatedAt: now,
      });
    }
    ...
    const workerRunner = {
      processClaimedItem: async (claimed: { id: string; loopId: string }) => {
