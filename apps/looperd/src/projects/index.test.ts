...
describe("ProjectManager", () => {
  ...
  test("preserves stored repo metadata when repo detection returns null", async () => {
    const rootDir = await mkdtemp(join(tmpdir(), "looper-projects-"));
    const store = new SqliteStore({
      dbPath: join(rootDir, "state", "looper.sqlite"),
      backupDir: join(rootDir, "backups"),
    ...
    const manager = new ProjectManager({
      ...
      git: {
        detectGitHubRepo: async () => {
          detectCalls += 1;
          return detectCalls === 1 ? "powerformer/looper" : null;
        },
        listWorktrees: async () => [],
      },
      github: {
    ...
    expect(store.projects.getById("looper")?.metadataJson).toContain(
